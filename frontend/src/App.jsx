...
function AppContent({ currentPage, onGetStarted, stacks, setStacks, selectedStack, setSelectedStack, setCurrentPage, onCreateStack }) {
        ...
        <div className="stacks-grid">
          {!stacks.length && (
            <div className="stack-card empty-state">
              ...
              <button className="new-stack-btn" onClick={onCreateStack}>+ New Stack</button>
            </div>
          )}
          {stacks.map((stack) => (
        ...
        {currentPage === 'landing' ? (
          ...
          <>
            <header style={{ backgroundColor: '#444', color: '#fff', padding: '10px 20px' }}>
              <h2>GenAI Stack</h2>
              <button className="new-stack-btn" onClick={handleNewStackClick} style={{ backgroundColor: '#2ecc71', color: '#fff' }}>+ New Stack</button>
            </header>
            {currentPage === 'genai-stack' ? (
              <ReactFlowProvider>
                <AppContent 
                  ...
                  onCreateStack={handleNewStackClick}
                />
              </ReactFlowProvider>
            ) : (
              <AppContent 
